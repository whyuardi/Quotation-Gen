      ...
      {!loading && products.length > 0 && (
        <>
          ...
          <div className="block md:hidden space-y-3">
            {products.map((p) => (
              <div key={p.id} className="bg-zinc-900/40 border border-zinc-800/80 rounded-xl p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <h3 className="text-zinc-200 hover:text-white font-medium">
                    {p.name}
                  </h3>
                  <span className="font-mono text-xs bg-zinc-800 px-2 py-0.5 rounded text-zinc-400">
                    {p.modelCode}
                  </span>
                </div>
                ...
                <div className="text-xs text-zinc-500 space-y-1">
                  ...
                  <p>BOM items: <span className="text-zinc-400">{p.priceItems.length}</span> · Quotations: <span className="text-zinc-400">{p._count.quotations}</span></p>
                </div>

                <div className="pt-2 border-t border-zinc-800/60 flex items-center justify-end gap-2">
                  <Link
                    href={`/admin/products/${p.id}/edit`}
                    className="px-2 py-1 bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-zinc-200 rounded-lg text-xs"
                  >
                    Edit
                  </Link>
                  <button
                    onClick={() => handleDelete(p.id, p.name)}
                    className="px-2 py-1 bg-zinc-900/50 border border-zinc-800/50 text-zinc-500 hover:text-red-400 rounded-lg text-xs"
                  >
                    Delete
                  </button>
                </div>
